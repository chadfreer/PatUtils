...
private:
  ...
  const edm::EDGetTokenT<std::vector<ScoutingPhoton> > photons_token_;
  ...
  const edm::EDGetTokenT<std::vector<ScoutingPFJet> > jets_token_;
  const edm::EDGetTokenT<std::vector<ScoutingMuon> > muon_token_;
...
L1PrefiringWeightProducer::L1PrefiringWeightProducer(const edm::ParameterSet& iConfig)
    : photons_token_(consumes<std::vector<ScoutingPhoton> >(iConfig.getParameter<edm::InputTag>("ThePhotons"))),
    //: photons_token_(consumes<std::vector<pat::Photon> >(iConfig.getParameter<edm::InputTag>("ThePhotons"))),
      ...
      jets_token_(consumes<std::vector<ScoutingPFJet> >(iConfig.getParameter<edm::InputTag>("TheJets"))),
      muon_token_(consumes<std::vector<ScoutingMuon> >(iConfig.getParameter<edm::InputTag>("TheMuons"))),
...
void L1PrefiringWeightProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  //Jets
  const std::vector<ScoutingPFJet>& theJets = iEvent.get(jets_token_);
  //const std::vector<pat::Jet>& theJets = iEvent.get(jets_token_);

  //Muons
  const std::vector<ScoutingMuon>& theMuons = iEvent.get(muon_token_);
  ...
  for (const auto fluct : {fluctuations::central, fluctuations::up, fluctuations::down}) {
    if (!missingInputEcal_) {
      ...
      for (const auto& jet : theJets) {
        ...
        if (fabs(eta_jet) > 3.)
          continue;
    TLorentzVector jet_energy;
    jet_energy.SetPtEtaPhiM(jet.pt(), jet.eta(), jet.phi(), jet.m() );

        if (jetMaxMuonFraction_ > 0 && jet.muonEnergy()/jet_energy.E() > jetMaxMuonFraction_)
        ...
        if (useEMpt_)
          pt_jet *= (jet.photonEnergy()/jet_energy.E() + jet.electronEnergy()/jet_energy.E());
    ...
    if (!missingInputMuon_ && doMuons_) {
      for (const auto& muon : theMuons) {
        ...
        if (pt < 5 || !(muon.isGlobalMuon() || muon.isTrackerMuon()))
